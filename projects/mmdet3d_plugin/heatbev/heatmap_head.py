...
class HeatmapHead(BaseModule):
    def __init__(self,
        ...
        self.heatmap_decoder = build_head(heatmap_decoder)
    ...
    def forward_train(self, img_feats, img_metas, gt_labels_3d, gt_bboxes_3d):
        ...
        bev_embed = self.heatmap_encoder(
        ...
        bev_embed = bev_embed.transpose(-2, -1)                                 # (B, Ch, Hh*Wh)
        bev_embed = bev_embed.reshape(*bev_embed.shape[:-1], self.Hh, self.Wh)  # (B, Ch, Hh, Wh)

        heatmaps = self.heatmap_decoder(bev_embed)
    ...
    def forward_test(self, img_feats, img_metas, score_threshold):
        ...
        bev_embed = bev_embed.transpose(-2, -1)                                 # (1, Ch, Hh*Wh)
        bev_embed = bev_embed.reshape(*bev_embed.shape[:-1], self.Hh, self.Wh)  # (1, Ch, Hh, Wh)
        ...
        heatmaps = F.interpolate(heatmaps[:, None, ...], size=[self.Hb, self.Wb], mode='bilinear').squeeze(1)  # (1, 200, 200)
