...
class HeatmapSpatialCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        output = output.reshape(B, N, max_len, self.embed_dims)  # (B, N, L, Ch)

        slots = output.new_zeros([B, P, self.embed_dims])  # (B, P, Ch)
        for batch in range(B):
            idx_list_per_batch = idx_lists[batch]
        ...
        slots = slots / count                # (B, P, Ch)

        slots = self.output_proj(slots)  # (B, P, Ch)

        out = self.dropout(slots) + self.position_encoder(ref_3d)  # (B, P, Ch)
...
class HeatmapMultiScaleDeformableAttention(BaseModule):
    ...
    def init_layers(self):
        ...
        self.value_proj = nn.Linear(256, self.embed_dims)
